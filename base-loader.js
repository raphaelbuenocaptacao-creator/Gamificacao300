...
function parse(text){let rows=[],row=[],cell='',q=false;const first=text.split(/\r?\n/).find(x=>x.trim())||'';const delim=(first.match(/;/g)||[]).length>(first.match(/,/g)||[]).length?';':',';for(let i=0;i<text.length;i++){const c=text[i],n=text[i+1];if(c==='"'&&q&&n==='"'){cell+='"';i++;continue}if(c==='"'){q=!q;continue}if(c===delim&&!q){row.push(cell);cell='';continue}if((c==='\n'||c==='\r')&&!q){if(c==='\r'&&n==='\n')i++;row.push(cell);if(row.some(v=>v.trim()))rows.push(row);row=[];cell='';continue}cell+=c}if(cell||row.length){row.push(cell);if(row.some(v=>v.trim()))rows.push(row)}return rows}
...
fetch(URL,{cache:'no-store'}).then(r=>{if(!r.ok)throw Error('CSV '+r.status);return r.text()}).then(text=>{
 let rows=parse(text);if(rows.length<2)throw Error('CSV sem dados');
 // O export do FluxoHub tem duas linhas de título antes do cabeçalho real.
 const headerIndex=rows.findIndex(r=>r.some(v=>key(v)==='PROMOTOR DE MARKETING'));
 if(headerIndex<0)throw Error('Cabeçalho "Promotor de marketing" não encontrado');
 const h=rows[headerIndex].map(key);rows=rows.slice(headerIndex+1);
 ...
 const count=Number(sessionStorage.getItem('arena_base_records')||0);sessionStorage.setItem('arena_base_records',String(out.length));
 if(count!==out.length && !sessionStorage.getItem('arena_base_reloaded')){sessionStorage.setItem('arena_base_reloaded','1');location.reload();}else{window.dispatchEvent(new CustomEvent('arena-base-ready',{detail:{records:out.length}}));}
