...
function parse(text){let rows=[],row=[],cell='',q=false;const lines=text;const first=(lines.split(/\r?\n/).find(x=>x.trim())||'');const delim=(first.match(/;/g)||[]).length>(first.match(/,/g)||[]).length?';':',';for(let i=0;i<lines.length;i++){const c=lines[i],n=lines[i+1];if(c==='"'&&q&&n==='"'){cell+='"';i++;continue}if(c==='"'){q=!q;continue}if(c===delim&&!q){row.push(cell);cell='';continue}if((c==='\n'||c==='\r')&&!q){if(c==='\r'&&n==='\n')i++;row.push(cell);if(row.some(v=>v.trim()))rows.push(row);row=[];cell='';continue}cell+=c}if(cell||row.length){row.push(cell);if(row.some(v=>v.trim()))rows.push(row)}return rows}
...
fetch(URL,{cache:'no-store'}).then(r=>{if(!r.ok)throw Error('CSV '+r.status);return r.text()}).then(text=>{
 ...
 const old=localStorage.getItem('arena_base_info');
 ...
 if(!old || JSON.parse(old).records!==out.length){sessionStorage.removeItem('arena_base_reloaded');}
 if(!sessionStorage.getItem('arena_base_reloaded')){sessionStorage.setItem('arena_base_reloaded','1');location.reload();}else{window.dispatchEvent(new CustomEvent('arena-base-ready',{detail:{records:out.length}}));}
